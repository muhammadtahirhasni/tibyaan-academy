...
export function TestimonialsSection() {
  ...
  const reviewsI18n = ((messages.testimonials as Record<string, unknown>)?.reviews ?? []) as Array<{
    review: string;
    role: string;
    course: string;
  }>;

  // Merge: use translated text if available, otherwise keep original English data
  const localizedReviews: Review[] = reviews.map((r, i) => ({
    ...r,
    review: reviewsI18n[i]?.review || r.review,
    role: reviewsI18n[i]?.role || r.role,
    course: reviewsI18n[i]?.course || r.course,
  }));

  const row1 = localizedReviews.slice(0, 24);
  const row2 = localizedReviews.slice(24);
